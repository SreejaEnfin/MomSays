...
export async function sendWelcomeEmail(to: string, name: string) {
    ...
    const htmlContent = ejs.render(templateContent, { name, S3_URL: process.env.S3_URL });
...
export async function childWelcomeEmail(to: string, name: string, alias: string) {
    ...
    const htmlContent = ejs.render(templateContent, { name, alias, S3_URL: process.env.S3_URL });
...
export async function passwordResetEmail(to: string, name: string, resetLink: string) {
    ...
    const htmlContent = ejs.render(templateContent, { name, resetLink, S3_URL: process.env.S3_URL });
...
export async function afterPasswordResetEmail(to: string, name: string) {
    const transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            user: process.env.GOOGLE_APP_USER,
    ...
    const templatePath = path.join(process.cwd(), 'src/utils/emailTemplates/afterpasswordreset.ejs');
    const templateContent = fs.readFileSync(templatePath, 'utf-8');
